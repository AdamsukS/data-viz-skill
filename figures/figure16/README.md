...
**synthetic_fitted_curve** — 按截图峰位与多峰形态估计的混合高斯示例轮廓；不是原始细胞表达数据
...
先确定细胞群和表达变换，再提供 marker/group/value 或 x/density。新数据的中位线由输入分布估计，不沿用截图中位线。
...
`synthetic_fitted_curve` · 11880 行。空值/NaN/Inf 不受支持；字段使用 UTF-8。
