...
| 23 | [时序函数生成与完整分布评估](../figures/figure23/README.md) | `deterministic_synthetic_inspired_by_published_layout` | 每个训练样本量对应的模型标识、随机种子、训练配置，以及每条生成函数的唯一 sample_id。 每条函数的坐标、生成值和按明确模型拟合或重建的函数值；同一 sample_id 必须共用坐标网格。 每条生成函数拟合得到的幅值 A、阻尼 gamma、角频率 omega、竖直位移 b，以及生成曲线与重建曲线之间的 MSE。 目标参数分布或可信参数边界；若真实任务没有解析参数，应替换为有定义的函数特征，而不是保留本例参数。 |
