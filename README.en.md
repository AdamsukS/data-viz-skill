...
**Let an AI reuse plotting code wherever practical and combine complementary perspectives in one figure.** This skill bundles 22 runnable Python chart templates with data, palettes, layouts, schemas and previews. Its core capability is connecting user data to existing code, preserving suitable layouts and adapting them as needed. When additional data would materially improve a useful layer, the agent suggests specific inputs such as individual experiment results, group/sample IDs and units. It proceeds with supported layers while optional inputs are unavailable and extends the existing figure for modest additions. Create a new composition only when substantial new variables or relationships cannot be expressed clearly through local edits, or when the user explicitly requests one. Data can come from business, engineering, surveys or any other domain.

## Use the skill

Copy this prompt directly to your agent; it includes the repository URL:

> Use the Data Viz Skill from https://github.com/AdamsukS/data-viz-skill. If it is not installed, download it using the repository README's command and read SKILL.md; otherwise use the installed $data-viz. Inspect my data and visualization requirements, then reuse and adapt the best-fitting template's Python code wherever practical. Preserve suitable layouts, palettes and information density. If repeated experiment results, repeated measurements or other inputs would improve a layer, specify the fields and their purpose; continue supported work while optional data are pending. Extend locally for modest additions. Create a new composition only if substantial new information cannot fit or I explicitly request it. For creative work, prefer complementary layers in one figure, such as observations, means, uncertainty and distributions, with data support and clear visual hierarchy. Deliver images, reusable Python code, data-format documentation and replay commands.
...
> Use $data-viz from https://github.com/AdamsukS/data-viz-skill to directly reuse the closest multi-panel template for these device metrics. Tell me which per-measurement fields would let you overlay observations and appropriate variability on the existing chart. Omit unsupported layers for now while preserving the remaining layout.
...
Missing data are not zeros: when only means are available, suggest supplying individual experiment results and experiment/sample IDs to enable observation points and justified variability summaries. Distinguish independent experiments from repeated readings of one sample. If additional data remain unavailable, keep means without unsupported points/errors; a missing inset should remove its CSV-loading code too. The renderer does not automatically support every missing-column case. The agent must adapt the copied code's data dependencies instead of inserting fictional values to preserve appearance.

Creative adaptations should retain this visual language: **overlay complementary perspectives in one figure**, such as observations + means + uncertainty, distribution shape + box summaries + individual values, or scatter relationships + fitted trends + marginal distributions. Use shared-coordinate overlays where compatible, and linked margins, insets or aligned panels where scales differ. Every layer needs a clear purpose and data support; use transparency, draw order and color to keep layers readable.
...
| 23 | [Temporal function generation and distribution recovery](figures/figure23/README.en.md) | `deterministic_synthetic_inspired_by_published_layout` |
...
![Temporal function generation and distribution recovery](figures/figure23/preview.png)

[Code and data documentation](figures/figure23/README.en.md)
...
```bash
python -m scripts.new_figure --id 24 --title "New chart" --title-en "New chart"
