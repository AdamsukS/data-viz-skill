...
**让 AI 尽量复用已有代码，在一张图中展示互补的信息视角。** 这个 Skill 包含 22 套可运行的 Python 图式，以及独立数据、配色、版式、字段说明和预览。核心能力是把用户数据接入现有代码，保留合适的布局并按需修改。缺少能显著改善展示的数据时，Agent 会提醒用户补充，例如多次实验的逐次结果、分组、样本 ID 和单位；暂时无法补充时，先绘制现有数据支持的部分。少量新增信息优先在原图上扩展。只有大量新变量或关系无法通过局部修改清晰表达，或用户明确要求新设计时，才创造新的构图。数据不限于生信，适用于业务、工程、调查及其他领域。
