...
**让 AI 尽量复用已有代码，在一张图中展示互补的信息视角。** 这个 Skill 包含 23 套可运行的 Python 图式，以及独立数据、配色、版式、字段说明和预览。核心能力是把用户数据接入现有代码，保留合适的布局并按需修改。缺少能显著改善展示的数据时，Agent 会提醒用户补充，例如多次实验的逐次结果、分组、样本 ID 和单位；暂时无法补充时，先绘制现有数据支持的部分。少量新增信息优先在原图上扩展。只有大量新变量或关系无法通过局部修改清晰表达，或用户明确要求新设计时，才创造新的构图。数据不限于生信，适用于业务、工程、调查及其他领域。

## 如何使用

可以把下面这段话直接复制给 Agent，仓库地址也已包含在指令中：

> 请使用 https://github.com/AdamsukS/data-viz-skill 中的 Data Viz Skill。如果尚未安装，请按仓库 README 的命令下载，并阅读 SKILL.md；如果已安装，直接使用 $data-viz。根据我提供的数据和可视化要求，尽量复用并修改最合适模板的 Python 代码，保留合适的布局、配色和信息密度。如果补充多次实验结果、重复测量或其他数据能改善展示，请具体说明需要哪些字段及其用途；在等待可选数据时先完成已有数据支持的部分。少量新增信息优先局部扩展。只有大量新增信息无法容纳，或我明确要求时，才创造新图。需要发挥创意时，优先在同一张图中叠加互补视角，例如原始观测点、均值、误差和分布；保证各层都有数据支持且清晰可辨。请交付图片、可复用的 Python 代码、数据格式说明和运行命令。

具体任务示例：

> 使用 https://github.com/AdamsukS/data-viz-skill 提供的 $data-viz 分析这份销售数据，选择适合展示地区差异和月度趋势的图式，输出图片与可替换数据的 Python 代码。

> 使用 https://github.com/AdamsukS/data-viz-skill 提供的 $data-viz，直接复用最接近这些设备指标的多面板模板代码。如果能补充各次测量值，请告诉我需要哪些字段，以便在原图上叠加观测点与适当的误差展示；暂时没有的数据可以略去，保留其余布局。
...
缺失数据不等于零：只有均值时，可以先提醒用户提供各次实验结果及实验/样本 ID，以便叠加散点并计算适当的波动摘要；还需区分独立实验与同一样本的重复读数。无法补充时保留均值，去掉缺少依据的散点与误差条；缺少插图数据时同步移除其 CSV 读取逻辑。现有渲染器不会自动处理所有缺列情况，Agent 需要在复制的代码中调整数据依赖，不能填入虚构值来维持外观。

创意扩展延续这套图的特点：**在一张图中叠加多个互补视角**，例如“观测点 + 均值 + 误差”“分布形状 + 箱线摘要 + 个体数据”，或“散点关系 + 拟合趋势 + 边缘分布”。同一坐标系适合叠加时直接叠加；尺度不兼容时用紧密关联的边缘图、插图或对齐面板。每层信息都应有明确用途和真实数据支持，并通过透明度、层级与颜色保持可读性。
